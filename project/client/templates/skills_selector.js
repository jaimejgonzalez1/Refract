...
Template.skills_selector.rendered = function(){
    ...
    'change .skill-input'(evt, wut) {
        ...
        if (checked) {
            if (Meteor.user().profile.skills_arr !== undefined) {
                // push into array
                Meteor.users.update({_id: Meteor.userId()}, {$push:{"profile.skills_arr":this.id}});
            } else {
                Meteor.users.update
                // create array
                ...
                // pluck from array
                Meteor.users.update({_id: Meteor.userId()}, {$pull:{"profile.skills_arr":this.id}});
            }
        }
    });
